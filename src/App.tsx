...
const SWEATS_CATEGORIES: SweatsCategory[] = [
...
const App = () => {
  ...
  const handleSendMessage = async () => {
    ...
    setTimeout(() => {
      const responses = [
        "That's great insight! Let's break that down into actionable SWEATS. Which category feels most pressing right now?",
        "I hear you. Remember, your Vital wave is connected to Energy activities. Even 15 minutes of movement can shift your momentum.",
        "Let's focus on that. I'll add it to your Work SWEATS for today. What's the first small step you can take?",
        "Your awareness of this is powerful. The Synthesis process is about aligning your daily actions with your deeper WAVES. What would make you feel more Accomplished today?",
      ];
...
const DashboardView = ({ tasks, toggleTask }: DashboardViewProps) => (
...
const SynthesisView = ({ messages, userInput, setUserInput, onSend, isLoading }: SynthesisViewProps) => {
        ...
        {isLoading && (
          <div className="flex justify-start">
            <div className="bg-white p-4 rounded-2xl border border-slate-100 flex gap-2">
              ...
              <span className="w-2 h-2 bg-slate-300 rounded-full animate-bounce" style={{ animationDelay: '75ms' }}></span>
...
interface SweatsViewProps {
  tasks: Task[];
  setTasks: React.Dispatch<React.SetStateAction<Task[]>>;
}

const SweatsView = ({ tasks }: SweatsViewProps) => {
...
const TrackerView = ({ active, category, setCategory, elapsed, toggle, format }: TrackerViewProps) => (
